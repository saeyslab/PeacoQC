...
#' ff_cleaned <- RemoveMargins(ff, channels, channel_specifications = channel_specifications)
#'
...
#'
#' #Run PeacoQC
#' PeacoQC_res <- PeacoQC(ff, channels, determine_good_cells = "all", plot = TRUE,
#'                save_fcs = TRUE)
#'
#' @export
PeacoQC <- function(ff,
  ...
  if (determine_good_cells %in% c("all", "IT", "MAD")){
    ...
    if (report == TRUE & !is.null(output_directory)){
      ...
      if (!is.null(weird_channel_decreasing) & !is.null(weird_channel_increasing)){
      ...
      utils::write.table(t(c(basename(ff@description$FILENAME), nrow(ff),
        ...
        perc_IT, MAD, IT_limit, consecutive_bins, events_per_bin, changing_channel)),
...
#'
#' @description \code{PeacoQCHeatmap} will make a heatmap to display all the results generated by \code{PeacoQC}. It will include the percentages of measurements that are removed in total, by the IT method and by the MAD method. It will also show the parameters that were used during the quality control.
#'
#' @usage
#' PeacoQCHeatmap(report_location, show_values = TRUE, show_row_names = TRUE,
#' latest_tests = FALSE, title = "PeacoQC report", ...)
#'
#' @param report_location The path to the PeacoQC report generated by \code{PeacoQC}.
#' @param show_values If set to TRUE, the percentages of removed values will be displayed on the heatmap.
#' @param show_row_names If set to FALSE, the filenames will not be displayed on the heatmap.
#' @param latest_tests If this is set to TRUE, only the latest quality control run will be displayed in the heatmap.
#' @param title The title that should be given to the heatmap.
#' @param ... Extra parameters to be given to the \code{Heatmap} function (eg. row_split)
#' @return This function returns nothing but generates a png file in the output_directory with the heatmap.
#'
...
PeacoQCHeatmap <- function(
  ...
  if(show_row_names == FALSE & latest_tests == FALSE)
    warning("If there are duplicates in the report file, they will be displayed on the heatmap without their filename.")
  ...
  if (latest_tests == TRUE){
    report_table <- report_table[!duplicated(report_table$Filename, fromLast = T),]
  ...
  if("No increasing or decreasing effect" %in% report_table$`Increasing/Decreasing channel`){
    col_incr_decr_channel <- c(col_incr_decr_channel, "No increasing or decreasing effect" = "#26C485")
  }
  if ("Increasing channel" %in% report_table$`Increasing/Decreasing channel`){
    col_incr_decr_channel <- c(col_incr_decr_channel, "Increasing channel" = "#AF3800")
  }
  if ("Decreasing channel" %in% report_table$`Increasing/Decreasing channel`){
    col_incr_decr_channel <- c(col_incr_decr_channel, "Decreasing channel" = "#721817")
  }
  if ("Increasing and decreasing channel" %in% report_table$`Increasing/Decreasing channel`){
    col_incr_decr_channel <- c(col_incr_decr_channel, "Increasing and decreasing channel" = "#A50104")
  }


  annotation_right <- rowAnnotation("Increasing/Decreasing channel" = as.factor(report_table[,"Increasing/Decreasing channel"]), col = list("Increasing/Decreasing channel" = col_incr_decr_channel))
