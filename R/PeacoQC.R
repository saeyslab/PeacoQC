...
#'
#' @usage
#' PeacoQCHeatmap(report_location, heatmap_name = "PeacoQC_heatmap",
#' display_plot = FALSE, output_directory = ".",
#' latest_tests = FALSE, title = "PeacoQC report")
#'
#' @param report_location The path to the PeacoQC report generated by \code{PeacoQC}.
#' @param heatmap_name The name that png file will have with the heatmap.
#' @param display_plot If set to TRUE, the heatmap will be displayed in the R environment.
#' @param output_directory Directory where the heatmap will be generated. If set to NULL, the heatmap will not be saved as png file.
...
PeacoQCHeatmap <- function(
  ...
  if(display_plot == FALSE & is.null(output_directory))
    stop("Both the display_plot and output_directory parameters don't allow for the generation of the heatmap.")
  ...
  heatmap <- pheatmap(report_table[,c(3,5,6)],
    cluster_cols = FALSE,
    cluster_rows = FALSE,
    main = title,
    display_numbers = TRUE,
    legend = F,
    annotation_row = annotation_frame, silent = TRUE)[[4]]
  ...
  if (!is.null(output_directory)){
    grDevices::pdf(file = file.path(output_directory, paste0(heatmap_name, ".pdf")),
      bg = "white",
      height = 7,
      width = 7)
