...
control result of only one sample and the visualisation of the results of multiple
...
The pipeline starts with a flowframe (flow cytometry or mass cytometry data) and
will first remove margins, then it will compensate and transform while ending
with the PeacoQC quality control. This will give back a list with the cleaned
flowframe and the different parameter settings, it will also save the flowframe
in a new fcs file and it will make a plot of the quality control for this sample.
...
ff <- flowWorkspace::compensate(ff, ff@description$SPILL)
ff <- flowWorkspace::transform(ff, 
  flowCore::estimateLogicle(ff, colnames(ff@description$SPILL)))
...
# Filtered flowframe is stored in peacoqc_res$FinalFF and can be used for further 
# analysis.
...
plot_path <- system.file("extdata", "PeacoQC_111_Comp_Trans.png", package = "PeacoQC")
...
# You can also change the MAD parameter to a higher value (to make it more strict)
# or to a lower value (to make it less strict). Since the MAD analysis does not
# remove something, this is not neccesary now.
...
#   ff <- flowWorkspace::compensate(ff, ff@description$SPILL)
#   ff <- flowWorkspace::transform(ff, flowCore::estimateLogicle(
#                                             ff, colnames(ff@description$SPILL)))
#   peacoqc_res <- PeacoQC(
#     ff, 
#     channels, 
#     determine_good_cells = "all",
